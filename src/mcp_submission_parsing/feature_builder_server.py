...
FEATURE_COLUMNS = [
    ...
    'umbrella_limit', 'capital_gains', 'capital_loss', 'credit_score',
    'telematics_score', 'incident_type', 'collision_type', 'incident_severity',
    'authorities_contacted', 'incident_state', 'incident_location',
    'incident_hour_of_the_day', 'number_of_vehicles_involved', 'property_damage',
    'bodily_injuries', 'witnesses', 'police_report_available', 'total_claim_amount',
    'prior_claims_count', 'auto_make', 'auto_year', 'injury_claim', 'property_claim',
    'vehicle_claim', 'incident_in_policy_period', 'policy_status_at_incident',
    'incident_near_boundary', 'is_complex_claim',
]

# Only used as absolute fallback (should never be needed)
DEFAULTS = {col: '' for col in FEATURE_COLUMNS}
DEFAULTS.update({
    'insured_sex': 'MALE', 'insured_education_level': 'Bachelors',
    'insured_occupation': 'other-service', 'insured_hobbies': 'reading',
    'insured_relationship': 'not-in-family', 'policy_state': 'CA',
    'policy_csl': '100/300', 'policy_deductable': 500, 'policy_annual_premium': 1000.0,
    'umbrella_limit': 0, 'capital_gains': 0, 'capital_loss': 0, 'credit_score': 650,
    'telematics_score': 70.0, 'incident_type': 'Other', 'collision_type': '?',
    'incident_severity': 'Minor Damage', 'authorities_contacted': 'None',
    'incident_state': 'CA', 'incident_location': 'Highway', 'incident_hour_of_the_day': 12,
    'number_of_vehicles_involved': 1, 'property_damage': 'YES', 'bodily_injuries': 0,
    'witnesses': 0, 'police_report_available': 'YES', 'total_claim_amount': 1000.0,
    'prior_claims_count': 0, 'auto_make': 'Unknown', 'auto_year': 2015,
    'injury_claim': 0, 'property_claim': 0, 'vehicle_claim': 0,
    'incident_in_policy_period': 'Y', 'policy_status_at_incident': 'Active',
    'incident_near_boundary': 'N', 'is_complex_claim': 'N',
    'months_as_customer': 24, 'age': 40,
})
...
def build_feature_vector(
    ...
    # Customer profile
    if customer_profile:
        f['insured_sex'] = _get(customer_profile, 'insured_sex', DEFAULTS['insured_sex'])
        f['insured_education_level'] = _get(customer_profile, 'insured_education_level', DEFAULTS['insured_education_level'])
        f['insured_occupation'] = _get(customer_profile, 'insured_occupation', DEFAULTS['insured_occupation'])
        f['insured_hobbies'] = _get(customer_profile, 'insured_hobbies', DEFAULTS['insured_hobbies'])
        f['insured_relationship'] = _get(customer_profile, 'insured_relationship', DEFAULTS['insured_relationship'])
        ...
        # months_as_customer
        try:
            since = _get(customer_profile, 'customer_since_date', '') or _get(customer_profile, 'first_policy_date', '')
            inc = _get(parsed, 'incident_date', '')
            if since and inc:
                f['months_as_customer'] = max(1, (datetime.strptime(str(inc), '%Y-%m-%d') - datetime.strptime(str(since), '%Y-%m-%d')).days // 30)
            else:
        ...
        # age
        try:
            dob = _get(customer_profile, 'date_of_birth', '') or _get(customer_profile, 'customer_dob', '')
            inc = _get(parsed, 'incident_date', '')
            if dob and inc:
                f['age'] = int((datetime.strptime(str(inc), '%Y-%m-%d') - datetime.strptime(str(dob), '%Y-%m-%d')).days / 365.25)
            else:
                imputed_fields.append('age')
        except:
            imputed_fields.append('age')
    else:
        imputed_fields.extend(['insured_sex', 'insured_education_level', 'insured_occupation', 'insured_hobbies',
                               'insured_relationship', 'credit_score', 'capital_gains', 'capital_loss',
    ...
    # Parsed submission (includes claim breakdowns)
    if parsed:
        ...
        f['incident_hour_of_the_day'] = int(_get(parsed, 'incident_hour_of_the_day', DEFAULTS['incident_hour_of_the_day']))
        f['number_of_vehicles_involved'] = int(_get(parsed, 'number_of_vehicles_involved', DEFAULTS['number_of_vehicles_involved']))
        ...
        f['police_report_available'] = str(_get(parsed, 'police_report_available', DEFAULTS['police_report_available']))
        f['total_claim_amount'] = float(_get(parsed, 'total_claim_amount', DEFAULTS['total_claim_amount']))
        # Claim breakdowns (now passed from test_parser)
        f['vehicle_claim'] = float(_get(parsed, 'vehicle_claim', f['total_claim_amount']))
        f['injury_claim'] = float(_get(parsed, 'injury_claim', 0.0))
        f['property_claim'] = float(_get(parsed, 'property_claim', 0.0))
